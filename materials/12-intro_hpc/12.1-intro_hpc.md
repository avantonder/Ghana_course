...
### Questions:
- What is an HPC and how does it differ from a regular computer?
- What can an HPC be used for?
- How can I access NMIMR-UG's HPC?
:::
...
:::{.callout-tip}
### Key Points:
- An HPC consists of several computers connected in a network. These are called **nodes**: 
...
- The filesystem on an HPC is often split between a small (backed) **home directory**, and a large and high-performance (non-backed) **scratch space**. 
  - The user's home is used for things like configuration files and local software installation.
...
When working on an HPC it is important to understand what kinds of _resources_ are available to us. 
...
In most cases there will be an HPC administrator (or team), who you can reach out for help if you need to obtain more information about how your HPC is organised. 

Some of the questions you may want to ask when you start using an HPC are: 
...
## 12.1.3 Navigating your way through NMIMR-UG's HPC

:::{.callout-note}
### Storage location of the workshop directory/folder in NMIMR-UG's HPC

The workshop directory contains all the neccessary files needed to complete the workshop. This is located in a shared space in the HPC.
You may navigate and peep into the directory using the below command.

```bash
ls /hpc/data/vm-shared/workshop_files_Bact_Genomics_2023/
```

Note that, you always have to point to that directory to copy files access files from your home directory.
We will direct you through out th ecourse hoe=w to find your way into and out of that space.
