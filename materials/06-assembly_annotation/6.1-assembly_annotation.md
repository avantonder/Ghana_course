...
## 6.2 Activate the `assembly` environment
...
We're going to use `Shovill` to create our *de novo* genome assemblies.
...
## 6.6 Deactivate `assembly` environment

Now that we are done with all our analysis, let’s deactivate the `assembly` environment:
