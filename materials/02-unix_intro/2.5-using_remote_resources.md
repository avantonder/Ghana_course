...
:::::{.callout-important icon=false}
## ***Exercise 2.5.1:*** How do we get files back from the remote server?
...
:::{.callout}
```bash
scp -r sshuser@remote-machine:/home/user/workshop_files_Bact_Genomics_2022/02_unix_intro .
```
...
G26832.R1.fastq.gz                              100%  218MB  10.6MB/s   00:20    
...
```bash
rsync -avz 02_unix_intro sshuser@remote-machine:/home/user/workshop_files_Bact_Genomics_2022/02_unix_intro
```
...
:::::{.callout-important icon=false}
## ***Exercise 2.5.2:*** How to remove files using `rsync --delete`
...
## ***Solution:***> 
```bash
rsync -avz --delete 02_unix_intro sshuser@remote-machine:/home/user/workshop_files_Bact_Genomics_2022/02_unix_intro
```
:::
:::::
...
Sshfs is another way of using the same ssh protocol to share files in a slightly different way. This software allows us to connect the file system of one machine with the file system of another using a "mount point". Let's start by making a directory in `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro` to act as this mount point. Convention tells us to call it `mnt`.

```bash
mkdir /home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/mnt
```

Now we can run the `sshfs` command

```bash
sshfs sshuser@remote-machine:/home/user /home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/mnt/
```
...
:::{.callout}
```bash
cd /home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/mnt/
ls -l
```
...
:::::{.callout-important icon=false}
## ***Exercise 2.5.3:*** When should we use which tool?
