...
<!--FIXME: Add a session on symbolic link here (ln -s )-->
...
<!--FIXME: Change the file directory system to represent that of the Linux environment that would be used for the training-->
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.1:*** Exploring More `ls` Flags
...
instead of `5369`.
<!-- FIXME: Add the exact outputs from a Linux PC to the solution -->
:::
:::::


:::::{.callout-important icon=false}
## ***Exercise 2.2.2:*** Listing in Reverse Chronological Order
...
<!--FIXME: Edit this portion with the appropriate file and directory names respective to the Linux environment we will be using -->
...
ls -F Desktop
```
<!--FIXME: Add other directories found on the Desktop to the outpur below-->

```
...
workshop_files_Bact_Genomics_2022/
...
```
:::
...
Your output should be a list of all the files and sub-directories in your Desktop directory, including the `workshop_files_Bact_Genomics_2022` directory.
...
Now that we know the `workshop_files_Bact_Genomics_2022` directory is located in our Desktop directory, we can do two things.
...
:::{.callout}
```bash
ls -F Desktop/workshop_files_Bact_Genomics_2022
```
```
...
```bash
cd Desktop
cd workshop_files_Bact_Genomics_2022
cd 02_unix_intro
```

These commands will move us from our home directory onto our Desktop, then into
the `workshop_files_Bact_Genomics_2022` directory, then into the `02_unix_intro` directory.  You will notice that `cd` doesn't print anything.  This is normal.  Many shell commands will not output anything to the screen when successfully executed. But if we run `pwd` after it, we can see that we are now in `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro`.
If we run `ls` without arguments now, it lists the contents of `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro`, because that's where we now are:
...
```
```
/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro
```
:::
...
ls -F
```
<!--FIXME: Update output information if necessary-->
```
...
```
```
-bash: cd: workshop_files_Bact_Genomics_2022: No such file or directory
```
:::
...
Sure enough, if we run `pwd` after running `cd ..`, we're back in `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022`:
...
```
```
/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022
```
:::
...
```bash
cd Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro
```
...
This allows us to move to our `workshop_files_Bact_Genomics_2022` directory from anywhere on the filesystem (including from inside `02_unix_intro`).  To find the absolute path we are looking for, we can use `pwd` and then extract the piece we need to move to `workshop_files_Bact_Genomics_2022`.
...
```bash
cd /home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022
```
...
mean "the current user's home directory". For example, for your home directory,`/home/ubuntu`, then `~/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro` is equivalent to
`/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro`. This only works if it is the first character in the
...
----
Try it!
First navigate to `~/Desktop/workshop_files_Bact_Genomics_2022` (you should already be there).

```bash
cd ~/Desktop/workshop_files_Bact_Genomics_2022
```
...
you'll see you're back in `~/Desktop/workshop_files_Bact_Genomics_2022`.
Run `cd -` again and you're back in `~/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/bacteria_rpob`
:::
...
Let's say we are in the `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/` and we type:
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.3:*** Absolute vs Relative Paths
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.4:*** Relative Path Resolution
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.5:*** `ls` Reading Comprehension
...
Let's go back to our `workshop_files_Bact_Genomics_2022` directory on the Desktop and use `ls -F` to see what it contains:
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.6:*** Creating Files a Different Way
...
Returning to the `workshop_files_Bact_Genomics_2022` directory,
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.8:*** Moving and Copying
What is the output of the closing `ls` command in the sequence shown below?
You can try the commands starting from `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/mycobacteria_rpob`.

```bash
pwd
```
```
/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/mycobacteria_rpob
```
```bash
ls
```
...
:::{.callout collapse="true"}
## ***Solution:***
- We start in the `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/mycobacteria_rpob` directory, and create a new folder called `recombine`.
...
The tricky part here is where the file was copied to. Recall that `..` means "**go up a level**", so the copied file is now in `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro`.
- Notice that `..` is interpreted with respect to the current working directory, **not** with respect to the location of the file being copied. So, the only thing that will show using ls (in `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/mycobacteria_rpob`) is the recombine folder.

1. No, see explanation above.  `Mycobacterium_tuberculosis_H37Rv_rpob-saved.fasta` is located in `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro`
2. Yes
3. No, see explanation above.  `Mycobacterium_tuberculosis_H37Rv_rpob.fasta` is located at `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/mycobacteria_rpob/recombine`
4. No, see explanation above.  `Mycobacterium_tuberculosis_H37Rv_rpob-saved.fasta` is located at `/home/ubuntu/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro`
:::
:::::
...
Returning to the `workshop_files_Bact_Genomics_2022` directory, let's tidy up this directory by removing the `second_draft.txt` file we created. The Unix command we will use for this is `rm` (short for 'remove'):
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.9:*** Using `rm` Safely
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.10:*** Copy with Multiple Filenames
For this exercise, you can test the commands in the `workshop_files_Bact_Genomics_2022/02_unix_intro` directory.
...
Let's consider the `workshop_files_Bact_Genomics_2022/02_unix_intro/molecules` directory:
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.11:*** List filenames matching a pattern
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.12:*** More on Wildcards

Look into the `exp` directory in `~/Desktop/workshop_files_Bact_Genomics_2022/02_unix_intro/exp` that has the following structure:
...
:::{.callout collapse="true"}
## ***Solution:***
<!--FIXME: Hide and release this solution after the class to serve as a real exercise-->
...
:::::{.callout-important icon=false}
## ***Exercise 2.2.13:*** Reproduce a folder structure
...
Which of the following set of commands would achieve this objective? What would the other commands do? Try them out in the `workshop_files_Bact_Genomics_2022/02_unix_intro` directory.
